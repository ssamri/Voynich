...
import { LAB_SCHEMAS, replayExperiment, runLab, type LabKind } from '../science/lab.js';
...
import { GENERATORS, type GeneratorKind } from '../science/generators.js';
...
scienceRouter.post('/refs/generate', (req, res) => {
  const b = z
    .object({
      kind: z.enum(GENERATORS.map((g) => g.kind) as [GeneratorKind, ...GeneratorKind[]]),
...
scienceRouter.post('/run/:kind', async (req, res) => {
  const kind = req.params.kind as LabKind;
