...
import { selectLines, wordsOf, type CorpusFilter } from '../voynich/analysis.js';
import { counts, entropyOf, glyphUnits, rng, shuffle, type Alphabet } from './text.js';

/** Algorithme de Sukhotin : sépare les symboles en « voyelles » et « consonnes » par leurs voisinages. */
export function sukhotin(tokens: string[], alphabet: Alphabet = 'eva') {
  ...
  for (;;) {
    let best = -1;
    for (let i = 0; i < n; i++) if (!vowel[i] && sums[i] > 0 && (best < 0 || sums[i] > sums[best])) best = i;
    if (best < 0) break;
    vowel[best] = true;
    order.push(syms[best]);
    for (let i = 0; i < n; i++) if (!vowel[i]) sums[i] -= 2 * adj[i][best];
  }
...
export function hmm(tokens: string[], opts: { states?: number; iterations?: number; alphabet?: Alphabet; seed?: number; maxSymbols?: number } = {}) {
  const K = Math.min(Math.max(opts.states ?? 2, 2), 6);
  const rand = rng(opts.seed ?? 7);
  const seq: string[] = [];
  for (const t of tokens) {
    seq.push(...glyphUnits(t, opts.alphabet ?? 'eva'), '_');
    if (seq.length > (opts.maxSymbols ?? 30000)) break;
  }
  ...
  for (let it = 0; it < (opts.iterations ?? 40); it++) {
    ...
    for (let i = 0; i < K; i++) alpha[0][i] = pi[i] * B[i][obs[0]];
    ...
    for (let t = 1; t < T; t++) {
      ...
      for (let j = 0; j < K; j++) {
        let a = 0;
        for (let i = 0; i < K; i++) a += alpha[t - 1][i] * A[i][j];
    ...
    for (let t = T - 2; t >= 0; t--) {
      for (let i = 0; i < K; i++) {
        let b = 0;
        for (let j = 0; j < K; j++) b += A[i][j] * B[j][obs[t + 1]] * beta[t + 1][j];
        beta[t][i] = b * c[t];
      }
    }
    ...
    for (let t = 0; t < T; t++) {
      ...
      for (let i = 0; i < K; i++) {
        const gamma = g[i] / gs;
        if (t === 0) newPi[i] = gamma;
        newB[i][obs[t]] += gamma;
        if (t < T - 1) {
          gammaSum[i] += gamma;
          for (let j = 0; j < K; j++) newA[i][j] += alpha[t][i] * A[i][j] * B[j][obs[t + 1]] * beta[t + 1][j];
        }
      }
    }
...
export function wordStructure(tokens: string[], alphabet: Alphabet = 'eva-grouped') {
  ...
  for (const [w, c] of types) {
    ...
    for (const a of seen)
      for (const b of seen)
        if (a !== b && u.indexOf(a) < u.indexOf(b)) before.set(`${a}|${b}`, (before.get(`${a}|${b}`) ?? 0) + c);
  }
  const ordered: { a: string; b: string; consistency: number; count: number }[] = [];
  for (const [k, ab] of before) {
    const [a, b] = k.split('|');
    const ba = before.get(`${b}|${a}`) ?? 0;
    if (ab + ba >= 30 && ab >= ba) ordered.push({ a, b, consistency: ab / (ab + ba), count: ab + ba });
  }
  ordered.sort((x, y) => y.consistency - x.consistency || y.count - x.count);
  const top = (m: Map<string, number>) => [...m.entries()].sort((a, b) => b[1] - a[1]).slice(0, 25).map(([s, count]) => ({ s, count }));
  return {
    alphabet,
...
function sectionTokens(filter: CorpusFilter = {}): Tok[] {
  const out: Tok[] = [];
  for (const l of selectLines(filter)) for (const w of wordsOf([l])) out.push({ w, section: l.illustration, folio: l.folio });
  return out;
}

/** Mots-clés par concentration (Montemurro & Zanette, 2013) : un mot « porteur de sens » se regroupe dans certaines parties du texte. */
export function keywords(opts: { filter?: CorpusFilter; parts?: number; minFreq?: number; seed?: number; limit?: number } = {}) {
  const toks = sectionTokens(opts.filter);
  const P = opts.parts ?? 64;
  const N = toks.length;
  if (N < P * 10) throw new Error('Texte trop court pour cette analyse');
  const partOf = (i: number) => Math.min(P - 1, Math.floor((i / N) * P));
  const dist = (seq: string[]) => {
  ...
  const H = (arr: number[]) => entropyOf(new Map(arr.map((v, i) => [i, v])));
  ...
  const rows: { word: string; count: number; information: number; concentration: number; mainSection: string | null; folios: string[] }[] = [];
  for (const [w, arr] of real) {
    const count = arr.reduce((a, b) => a + b, 0);
    if (count < (opts.minFreq ?? 10)) continue;
...
export function similarWords(word: string, opts: { filter?: CorpusFilter; window?: number; minFreq?: number; limit?: number } = {}) {
  const words = wordsOf(selectLines(opts.filter ?? {}));
  const freq = counts(words);
  const minF = opts.minFreq ?? 5;
  if ((freq.get(word) ?? 0) < minF) throw new Error(`« ${word} » est trop rare (${freq.get(word) ?? 0} occurrences, minimum ${minF}).`);
  const W = opts.window ?? 2;
  const co = new Map<string, Map<string, number>>();
  let total = 0;
  for (let i = 0; i < words.length; i++) {
    const a = words[i];
    if ((freq.get(a) ?? 0) < minF) continue;
  ...
  const ctxTotals = new Map<string, number>();
  for (const m of co.values()) for (const [b, c] of m) ctxTotals.set(b, (ctxTotals.get(b) ?? 0) + c);
  ...
  const vec = (a: string) => {
    ...
    for (const [b, c] of co.get(a) ?? []) {
      ...
      if (pmi > 0) out.set(b, pmi);
    }
    return out;
  };
  const norm = (v: Map<string, number>) => Math.sqrt([...v.values()].reduce((a, x) => a + x * x, 0));
  const target = vec(word);
  const tn = norm(target);
  const sims: { word: string; similarity: number; count: number; editDistanceHint: boolean }[] = [];
  for (const a of co.keys()) {
    if (a === word) continue;
    const v = vec(a);
    let dot = 0;
    for (const [k, x] of target) dot += x * (v.get(k) ?? 0);
...
export function lineEffects(filter: CorpusFilter = {}) {
  const rows = db
    .prepare(
      `SELECT l.folio, l.locus, l.locus_type, l.text, p.illustration, p.language, p.hand FROM corpus_lines l LEFT JOIN corpus_pages p ON p.folio = l.folio ORDER BY l.ord`,
    )
    .all() as { folio: string; locus: string; locus_type: string | null; text: string; illustration: string | null; language: string | null; hand: string | null }[];
  const lines = rows.filter(
    (l) => (!filter.illustration || l.illustration === filter.illustration) && (!filter.language || l.language === filter.language) && (!filter.hand || l.hand === filter.hand),
  );
  const pos = { first: new Map<string, number>(), middle: new Map<string, number>(), last: new Map<string, number>() };
  const lenByPos = { first: [] as number[], middle: [] as number[], last: [] as number[] };
  const initialGlyph = { first: new Map<string, number>(), all: new Map<string, number>() };
  const finalGlyph = { last: new Map<string, number>(), all: new Map<string, number>() };
  let gallowsParaFirst = 0;
  let paraFirstLines = 0;
  let gallowsOther = 0;
  let otherLines = 0;
  const isParaStart = (t: string | null) => Boolean(t && /^@P|^\*P/.test(t));
  ...
  const ratio = (sub: Map<string, number>, all: Map<string, number>) => {
  ...
  const topW = (m: Map<string, number>) => [...m.entries()].sort((a, b) => b[1] - a[1]).slice(0, 12).map(([word, count]) => ({ word, count }));
  return {
    lines: lines.length,
