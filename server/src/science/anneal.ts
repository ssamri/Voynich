...
import { counts, glyphUnits, levenshtein, rng, shuffle, tokensOf, type Alphabet } from './text.js';

/**
 * Recherche automatique d'une clé de substitution (unités EVA → lettres) par recuit simulé,
 * en maximisant la vraisemblance du texte obtenu sous un modèle trigramme de la langue cible.
 *
 * Point clé : un optimiseur trouve TOUJOURS une clé qui « ressemble » à la langue. On lance donc la
 * même optimisation sur un texte de contrôle (glyphes de Voynich mélangés, mêmes longueurs de mots).
 * Seul un écart net entre le vrai texte et le contrôle constitue un signal.
 */
export interface AnnealInput {
  ...
  maxWords?: number;
  seed?: number;
  cribs?: { eva: string; expected: string; weight?: number }[];
  withControl?: boolean;
  fixed?: Record<string, string>;
  /** Contrôles supplémentaires : textes générés ou autres langues chiffrées, optimisés de la même façon. */
  controlSources?: { name: string; tokens: string[] }[];
}
...
class TrigramScorer {
  ...
  constructor(text: string) {
    ...
    for (let a = 0; a < L; a++)
      for (let b = 0; b < L; b++)
        for (let c = 0; c < L; c++) this.table[(a * L + b) * L + c] = Math.log2((tri[(a * L + b) * L + c] + 0.1) / (bi[a * L + b] + 0.1 * L));
  }

  letterIndex(l: string) {
    return this.idx.get(l) ?? -1;
  }

  /** Log-vraisemblance moyenne (bits/caractère, négatif) d'une séquence d'indices (0 = frontière de mot). */
  score(seq: Int32Array) {
    const L = this.L;
    let s = 0;
    for (let i = 2; i < seq.length; i++) s += this.table[(seq[i - 2] * L + seq[i - 1]) * L + seq[i]];
    return s / (seq.length - 2);
  }
}

const scorerCache = new Map<number, TrigramScorer>();
function scorerFor(refId: number) {
  if (!scorerCache.has(refId)) {
    const ref = getRef(refId);
    if (!ref || ref.kind !== 'natural') throw new Error('Choisissez un corpus de référence en langue naturelle.');
    scorerCache.set(refId, new TrigramScorer(tokensOf(ref.text).slice(0, 300000).join(' ')));
  }
  return scorerCache.get(refId)!;
}

function optimise(words: string[][], scorer: TrigramScorer, input: AnnealInput, rand: () => number) {
  ...
  const evaluate = (key: Int32Array) => {
    let k = 0;
    buf[k++] = 0;
    for (const w of encoded) {
      for (const u of w) buf[k++] = key[u];
      buf[k++] = 0;
    }
  ...
  let best: { key: Int32Array; score: number } | null = null;
  for (let r = 0; r < (input.restarts ?? 3); r++) {
    ...
    for (let i = 0; i < units.length; i++) key[i] = 1 + ((i + (r ? Math.floor(rand() * 4) : 0)) % letters.length);
    for (const [u, l] of fixed) key[u] = l;
    ...
    for (let it = 0; it < iters; it++) {
      ...
      if (s >= cur || rand() < Math.exp((s - cur) / T)) {
        cur = s;
        if (s > localBest.score) localBest = { key: key.slice(), score: s };
      } else {
        key[u] = old;
        if (v >= 0) key[v] = oldV;
      }
...
export function anneal(input: AnnealInput) {
  ...
  if (tokens.length < 100) throw new Error('Texte trop court pour l’optimisation (100 mots minimum).');
  const words = tokens.map((t) => glyphUnits(t, input.alphabet ?? 'eva'));
  const real = optimise(words, scorer, input, rand);

  let control: ReturnType<typeof optimise> | null = null;
  ...
  for (const w of refSample) {
    for (const c of w) buf.push(scorer.letterIndex(c));
    buf.push(0);
  }
