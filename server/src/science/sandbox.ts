...
export async function runSandbox(code: string, opts: { refs?: number[]; timeoutMs?: number } = {}) {
  ...
  try {
    ...
    const refs: Record<number, { name: string; language: string | null; words: string[] }> = {};
    for (const id of opts.refs ?? []) {
      const r = getRef(id);
      if (r) refs[id] = { name: r.name, language: r.language, words: tokensOf(r.text).slice(0, 60000) };
    }
    ...
    const logs = outHandle.error ? '' : (ctx.dump(outHandle.value) as string);
    if (outHandle.error) outHandle.error.dispose();
    else outHandle.value.dispose();
    if (res.error) {
      const e = ctx.dump(res.error) as { name?: string; message?: string };
      res.error.dispose();
