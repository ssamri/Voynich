...
import { generate, type GeneratorKind } from './generators.js';
import { CharNgramModel } from './ngram.js';
import { normalizeText, tokensOf } from './text.js';

export interface RefCorpusRow {
  id: number;
  name: string;
 ...
 */
export function parseVoynichSource(src: string): CorpusFilter | null {
  if (src === 'voynich') return {};
  const m = /^voynich:(lang|section|hand):(\w+)$/.exec(src);
  if (!m) return null;
  return m[1] === 'lang' ? { language: m[2] } : m[1] === 'section' ? { illustration: m[2] } : { hand: m[2] };
}

export function tokensFor(src: string): { id: string; name: string; kind: string; tokens: string[] } {
  const vf = parseVoynichSource(src);
  if (vf) return { id: src, name: src === 'voynich' ? 'Voynich (tout)' : `Voynich ${src.slice(8)}`, kind: 'voynich', tokens: wordsOf(selectLines(vf)) };
  const m = /^ref:(\d+)$/.exec(src);
  if (!m) throw new Error(`Source inconnue : ${src}`);
  const row = getRef(Number(m[1]));
  if (!row) throw new Error(`Corpus de référence #${m[1]} introuvable`);
...
export function listRefs() {
  return db
    ...
    .all() as Omit<RefCorpusRow, 'text'>[];
}

export function getRef(id: number) {
  return db.prepare('SELECT * FROM ref_corpora WHERE id = ?').get(id) as RefCorpusRow | undefined;
}

export function deleteRef(id: number) {
  db.prepare('DELETE FROM ref_corpora WHERE id = ?').run(id);
  modelCache.delete(id);
  fpCache.delete(`ref:${id}`);
}

export function addRef(r: { name: string; language?: string; genre?: string; kind: RefCorpusRow['kind']; source?: string; notes?: string; params?: unknown; tokens: string[] }) {
  if (r.tokens.length < 200) throw new Error('Texte trop court (200 mots minimum) pour des statistiques fiables.');
  const info = db
...
export async function importRefFromUrl(url: string, meta: { name: string; language?: string; genre?: string; medievalLatin?: boolean; stripDiacritics?: boolean }) {
...
export function importRefFromText(raw: string, meta: { name: string; language?: string; genre?: string; source?: string; medievalLatin?: boolean; stripDiacritics?: boolean }) {
...
function stripGutenberg(text: string) {
...
export function generateRef(kind: GeneratorKind, opts: { sourceRefId?: number; voynichSource?: string; length?: number; seed?: number; name?: string }) {
  ...
  if (['simple_substitution', 'homophonic', 'verbose_cipher'].includes(kind) && !ref) throw new Error('Choisissez un corpus de référence en langue naturelle à chiffrer.');
  if (['shuffle_words', 'shuffle_chars', 'rugg_grille', 'timm_autocopy'].includes(kind) && !voynichTokens.length) throw new Error('Importez d’abord le corpus EVA.');
...
const modelCache = new Map<number, CharNgramModel>();
export function languageModel(refId: number, order = 4) {
  const hit = modelCache.get(refId);
  if (hit && hit.order === order) return hit;
  const ref = getRef(refId);
  if (!ref) throw new Error(`Corpus de référence #${refId} introuvable`);
  const m = CharNgramModel.fromText(ref.text, order);
  modelCache.set(refId, m);
  return m;
}

export function vocabulary(refId: number) {
  const ref = getRef(refId);
  if (!ref) throw new Error(`Corpus de référence #${refId} introuvable`);
...
export function fingerprintFor(src: string) {
  // Le corpus Voynich peut changer (réimport) : pas de cache pour lui.
  if (src.startsWith('ref:') && fpCache.has(src)) return fpCache.get(src)!;
  const fp = fingerprint(tokensFor(src).tokens);
  if (src.startsWith('ref:')) fpCache.set(src, fp);
  return fp;
}
