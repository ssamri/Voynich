...
export class CharNgramModel {
  ...
  private contextTotals: Map<string, number>[] = [];
  private contextTypes: Map<string, number>[] = [];
  readonly alphabet = new Set<string>();

  constructor(readonly order = 4) {
  ...
  static fromText(text: string, order = 4) {
  ...
  train(text: string) {
    ...
    for (const ch of s) this.alphabet.add(ch);
    const chars = [...s];
    for (let i = 0; i < chars.length; i++) {
      for (let n = 1; n <= this.order; n++) {
        ...
        if (prev === 0) this.contextTypes[n].set(ctx, (this.contextTypes[n].get(ctx) ?? 0) + 1);
      }
    }
  }

  prob(ctx: string[], ch: string): number {
  ...
  crossEntropy(text: string): number {
    ...
    if (chars.length < 3) return Infinity;
    let bits = 0;
