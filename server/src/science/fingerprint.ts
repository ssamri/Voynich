...
/**
 * Empreinte statistique d'un texte, calculée sur des échantillons de taille fixe
 * pour que des textes de longueurs différentes restent comparables.
 */
export interface Fingerprint {
  ...
  zipfSlope: number;
  repeatRate: number;
  positionalSpecialization: number;
  wordBigramEntropy: number;
  wordLengths: number[];
}

export const FEATURES: { key: keyof Fingerprint; label: string; hint: string }[] = [
...
export function fingerprint(tokens: string[]): Fingerprint {
  ...
  for (const l of lengths) wl[Math.min(l, 15)]++;
 ...
 */
export function compareFingerprints(target: { id: string; name: string; fp: Fingerprint }, refs: { id: string; name: string; kind: string; fp: Fingerprint }[]) {
  const keys = FEATURES.map((f) => f.key).filter((k) => k !== 'alphabetSize');
  const all = [target.fp, ...refs.map((r) => r.fp)];
  const stats = Object.fromEntries(
    keys.map((k) => {
      const vals = all.map((fp) => fp[k] as number);
      return [k, { m: mean(vals), s: sd(vals) || 1 }];
    }),
  );
  const z = (fp: Fingerprint, k: keyof Fingerprint) => ((fp[k] as number) - stats[k].m) / stats[k].s;
  const ranking = refs
    .map((r) => {
