...
import { createMemory, type MemoryType } from '../memory.js';

/**
 * Socle de connaissances : un siècle de recherche résumé, pour que les agents
 * ne repartent pas de zéro et ne retombent pas dans les impasses connues.
 * Chaque élément cite sa source ; les confiances restent prudentes.
 */
interface Seed {
  type: MemoryType;
  title: string;
...
export function seedKnowledge() {
  const existing = new Set((db.prepare(`SELECT title FROM memories`).all() as { title: string }[]).map((r) => r.title));
  let created = 0;
  for (const s of SEEDS) {
    ...
    if (s.status && s.status !== 'active') db.prepare('UPDATE memories SET status = ? WHERE id = ?').run(s.status, m.id);
    created++;
  }
  const hasBiblio = db.prepare(`SELECT id FROM documents WHERE title = ?`).get('Bibliographie de référence — Manuscrit de Voynich');
  if (!hasBiblio) createTextDocument('Bibliographie de référence — Manuscrit de Voynich', BIBLIOGRAPHY, 'socle,bibliographie', 'Voynich Lab');
