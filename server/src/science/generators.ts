...
/**
 * Générateurs de textes de contrôle. Ils servent de points de comparaison :
 * si un texte généré mécaniquement reproduit les statistiques de Voynich aussi bien
 * qu'une langue naturelle, l'hypothèse « langue naturelle » perd de sa force.
 */
...
export const GENERATORS: { kind: GeneratorKind; label: string; needsSource: 'voynich' | 'reference' | 'none'; description: string }[] = [
...
function sizeLimit(tokens: string[], n: number) {
  return tokens.slice(0, Math.min(tokens.length, n));
}

export function generate(kind: GeneratorKind, opts: { voynichTokens?: string[]; referenceTokens?: string[]; length?: number; seed?: number }): string[] {
  ...
  switch (kind) {
    ...
    case 'rugg_grille': {
      ...
      const suf = new Map<string, number>();
      for (const w of vt) {
      ...
      const top = (m: Map<string, number>) => [...m.entries()].sort((x, y) => y[1] - x[1]).slice(0, 36).map(([s]) => s);
      ...
      while (out.length < length) {
        ...
        if (rand() < 0.05) offsets[Math.floor(rand() * 3)] = Math.floor(rand() * rows);
      }
      return out;
    }

    case 'timm_autocopy': {
      const subs: string[][] = [['o', 'a', 'y'], ['e', 'ee'], ['ch', 'sh'], ['k', 't', 'p', 'f'], ['d', 'l', 'r', 's'], ['n', 'm'], ['in', 'iin', 'ir'], ['ckh', 'cth']];
      const prefixes = ['q', 'qo', 'o', 'ch', 'sh', 'y', 'd'];
      const suffixes = ['y', 'dy', 'ey', 'ol', 'or', 'al', 'ar', 'aiin', 'in'];
      const out: string[] = sizeLimit(vt, 12).length ? sizeLimit(vt, 12) : ['daiin', 'chol', 'qokeedy', 'shedy', 'otar'];
      const mutate = (w: string) => {
        let s = w;
        const r = rand();
        if (r < 0.55) {
          ...
          if (from) s = s.replace(from, pick(group, rand));
        } else if (r < 0.7) {
      ...
      while (out.length < length) {
        ...
        if (src.length > 9) src = pick(out.filter((w) => w.length >= 3 && w.length <= 7).slice(-50).concat(['daiin', 'chol', 'shedy']), rand);
        let next = rand() < 0.1 ? src : mutate(src);
        if (next.length > 10) next = next.slice(0, 4 + Math.floor(rand() * 4));
        out.push(next);
      }
      return out;
    }
    ...
    case 'homophonic': {
      ...
      for (const l of letters) {
        ...
        if (!homophones.get(l)!.length) homophones.set(l, [pick(EVA_SYMBOLS, rand)]);
      }
