...
export function chunkText(text: string, size = 1600, overlap = 200) {
  const chunks: { start: number; content: string }[] = [];
  let start = 0;
  while (start < text.length) {
    ...
    if (content) chunks.push({ start, content });
...
function indexDocument(id: number, content: string) {
...
export async function ingestFile(file: { originalname: string; mimetype: string; buffer: Buffer }, meta: { title?: string; tags?: string; source?: string }) {
  const name = file.originalname;
  let kind: DocumentRow['kind'];
  let content = '';
  ...
  const tx = db.transaction(() => {
    const info = db
      .prepare(
        'INSERT INTO documents(title, filename, mime, kind, size, tags, source, content, file_path) VALUES (?,?,?,?,?,?,?,?,?)',
      )
      .run(meta.title?.trim() || name, name, file.mimetype, kind, file.buffer.length, meta.tags ?? '', meta.source ?? null, content, filePath);
    const id = Number(info.lastInsertRowid);
    if (content) indexDocument(id, content);
    return id;
  });
  return tx();
}

export function createTextDocument(title: string, content: string, tags = '', source: string | null = null) {
...
export function updateDocument(id: number, patch: { title?: string; tags?: string; notes?: string; content?: string }) {
  const doc = getDocument(id);
  if (!doc) throw new Error('Document introuvable');
  db.transaction(() => {
    db.prepare('UPDATE documents SET title = ?, tags = ?, notes = ?, content = ?, size = ? WHERE id = ?').run(
      patch.title ?? doc.title,
      patch.tags ?? doc.tags,
      patch.notes ?? doc.notes,
      patch.content ?? doc.content,
      patch.content !== undefined ? Buffer.byteLength(patch.content) : doc.size,
      id,
    );
    if (patch.content !== undefined) indexDocument(id, patch.content);
  })();
}
...
export function deleteDocument(id: number) {
  ...
  if (doc.file_path) fs.rmSync(path.join(config.uploadsDir, doc.file_path), { force: true });
}

export function documentFile(doc: DocumentRow) {
  return doc.file_path ? path.join(config.uploadsDir, doc.file_path) : null;
}

export function searchLibrary(query: string, limit = 8) {
  const q = ftsQuery(query);
  if (!q) return [];
  return db
    .prepare(
      `SELECT c.document_id, d.title, c.idx, c.start_offset,
    ...
    .all(q, limit) as { document_id: number; title: string; idx: number; start_offset: number; snippet: string; score: number }[];
}
