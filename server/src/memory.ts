...
export const MEMORY_TYPES = ['hypothesis', 'finding', 'fact', 'dead_end', 'glossary', 'question', 'plan'] as const;
...
export interface MemoryRow {
  ...
  tags: string;
  confidence: number;
  status: 'active' | 'confirmed' | 'refuted' | 'archived';
  pinned: number;
...
export function createMemory(m: {
  ...
  const info = db
    .prepare(
      `INSERT INTO memories(type, title, content, tags, confidence, pinned, author_agent_id, author_label, session_id)
       VALUES (?,?,?,?,?,?,?,?,?)`,
    )
    .run(
      m.type,
      ...
      m.content,
      m.tags ?? '',
      Math.min(1, Math.max(0, m.confidence ?? 0.5)),
      m.pinned ? 1 : 0,
      m.authorAgentId ?? null,
      m.authorLabel ?? null,
      m.sessionId ?? null,
    );
...
export function updateMemory(
  id: number,
  patch: Partial<Pick<MemoryRow, 'type' | 'title' | 'content' | 'tags' | 'confidence' | 'status'>> & { pinned?: boolean },
) {
  const cur = getMemory(id);
  if (!cur) throw new Error(`Mémoire #${id} introuvable`);
  db.prepare(
    `UPDATE memories SET type=?, title=?, content=?, tags=?, confidence=?, status=?, pinned=?, updated_at=datetime('now') WHERE id=?`,
  ).run(
    patch.type ?? cur.type,
...
export function searchMemories(query: string, opts: { type?: string; limit?: number; includeInactive?: boolean } = {}) {
  ...
  if (!q) {
    return db
      ...
      .all({ type: opts.type, limit }) as MemoryRow[];
  }
  return db
    .prepare(
    ...
    .all({ q, type: opts.type, limit }) as MemoryRow[];
}

export function pinnedMemories() {
  return db.prepare(`SELECT * FROM memories WHERE pinned = 1 AND status != 'archived' ORDER BY updated_at DESC LIMIT 30`).all() as MemoryRow[];
}

export function formatMemory(m: MemoryRow) {
