...
export async function hashPassword(password) {
    ...
    const hash = await new Promise((res, rej) => crypto.scrypt(password, salt, 64, SCRYPT, (err, key) => (err ? rej(err) : res(key))));
...
export async function verifyPassword(password, stored) {
    ...
    const actual = await new Promise((res, rej) => crypto.scrypt(password, Buffer.from(salt, 'base64url'), expected.length, { ...SCRYPT, N: Number(n) }, (err, key) => err ? rej(err) : res(key)));
