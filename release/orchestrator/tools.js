...
import { LAB_SCHEMAS, REGION, replayExperiment, runLab } from '../science/lab.js';
...
function tool(name, description, input, run) {
    const { $schema: _ignored, ...jsonSchema } = z.toJSONSchema(input);
    return { name, description, input: input, jsonSchema, run };
}
...
export function buildTools(ctx) {
    const tools = [];
    const has = (g) => ctx.groups.includes(g);
    if (has('library')) {
        tools.push(tool('search_library', 'Recherche plein texte dans la bibliothèque de documents (articles, livres, notes, transcriptions). Renvoie des extraits avec l’identifiant du document et la position.', z.object({ query: z.string().min(2).describe('Mots-clés'), limit: z.number().int().min(1).max(20).optional() }), ({ query, limit }) => {
            const hits = searchLibrary(query, limit ?? 8);
            if (!hits.length)
                return 'Aucun résultat dans la bibliothèque.';
            return hits.map((h) => `[doc #${h.document_id} « ${h.title} », offset ${h.start_offset}] ${h.snippet}`).join('\n\n');
        }), tool('read_document', 'Lit un passage d’un document de la bibliothèque (texte extrait). Utiliser offset pour paginer.', z.object({
            ...
            if (!doc)
                return `Document #${document_id} introuvable.`;
            if (doc.kind === 'image')
                return `Le document #${document_id} est une image (« ${doc.title} ») ; il est joint visuellement s’il fait partie du contexte de la séance. Notes : ${doc.notes ?? '—'}`;
    ...
    if (has('memory')) {
        tools.push(tool('search_memory', 'Recherche dans la mémoire partagée de l’équipe (hypothèses, découvertes, faits, impasses, glossaire, questions, plans).', z.object({
        ...
        }), tool('save_memory', 'Enregistre un élément durable dans la mémoire partagée. Pour une découverte (finding) ou un fait (fact), la preuve est OBLIGATOIRE : numéro d’expérience (#E12), URL, document de la bibliothèque (doc #3) ou référence bibliographique [Auteur, année].', z.object({
        ...
        }), tool('update_memory', 'Met à jour un élément de mémoire : statut (confirmed, refuted, archived), confiance ou complément. Passer une hypothèse en « confirmed » exige une expérience du juge au verdict PASS (#E<n>).', z.object({
            id: z.number().int(),
            ...
            if (!cur)
                return `Mémoire #${id} introuvable.`;
            if (status === 'confirmed' && cur.type === 'hypothesis') {
                const passing = experimentIds(evidence).map(getExperiment).filter((e) => e?.verdict === 'pass');
                if (!passing.length)
                    throw new Error('Confirmation refusée : il faut une expérience du juge avec le verdict PASS (evaluate_decipherment), citée comme #E<n>.');
            }
    ...
    if (has('corpus')) {
        const requireCorpus = () => {
            if (!corpus.corpusLoaded())
                throw new Error('Aucun corpus EVA importé. Demandez au chercheur de l’importer (page Corpus).');
        };
        tools.push(tool('corpus_info', 'Informations sur la transcription EVA chargée et la liste des folios avec section et langue.', z.object({}), () => {
            requireCorpus();
            const pages = listPages()
                .map((p) => `${p.folio}:${p.illustration ?? '?'}/${p.language ?? '?'}/${p.hand ?? '?'}`)
                .join(' ');
            return `${json(corpusInfo())}\n\nFolios (folio:section/langue/main) :\n${pages}`;
        }), tool('corpus_get_folio', 'Renvoie la transcription EVA d’un folio (ex. "f1r", "f68r3"), ligne par ligne, avec ses métadonnées (section, langue, main).', z.object({ folio: z.string().regex(/^f\d+[rv]\d*$/) }), ({ folio }) => {
            requireCorpus();
            const p = getPage(folio);
            if (!p)
                return `Folio ${folio} introuvable.`;
            return `${json(p.page)}\n\n${p.lines.map((l) => `${l.locus}: ${l.text}`).join('\n')}`;
        }), tool('corpus_search', 'Recherche un mot EVA exact ou une expression régulière JavaScript dans le corpus. Renvoie le nombre total d’occurrences et les lignes (locus).', z.object({
        ...
        }), tool('corpus_stats', 'Statistiques sur le corpus ou un sous-corpus : overview (tokens, types, entropies h0/h1/h2, mots fréquents, longueurs), positional (glyphes en début/milieu/fin de mot), ngrams (caractères ou mots), compare (sous-corpus A vs B).', z.object({
    ...
    if (has('substitution')) {
        tools.push(tool('apply_substitution', 'Teste une hypothèse de déchiffrement : applique une table de substitution (séquences EVA → lettres/syllabes) à un folio ou à un texte EVA. Les séquences les plus longues sont prioritaires. Renvoie le texte transformé et les glyphes non couverts.', z.object({
            ...
            if (!source)
                return 'Fournir folio ou text.';
            const lines = source.split('\n').map((l) => corpus.substitute(l, mapping));
            const unmapped = {};
            for (const l of lines)
                for (const [k, v] of Object.entries(l.unmapped))
    ...
    if (has('science')) {
        ...
        tools.push(tool('list_reference_corpora', 'Liste les corpus de comparaison (langues naturelles, chiffres, textes générés) avec leur identifiant « ref:<id> ».', z.object({}), () => {
            const refs = listRefs();
        ...
        }), lab('fingerprint', 'Compare l’empreinte statistique (entropies, longueurs, Zipf, positions, répétitions…) d’un texte avec des langues réelles, des chiffres et des textes générés. Élimine des familles d’hypothèses.'), tool('register_test', 'Pré-enregistre un test AVANT de l’exécuter : les critères de réussite sont figés. Renvoie un numéro à passer ensuite à evaluate_decipherment (test_id).', z.object({
        ...
        }), lab('evaluate', 'LE JUGE. Note une hypothèse de déchiffrement (table de substitution ou glossaire mot à mot) : couverture, plausibilité linguistique, mots du dictionnaire, comparaison à des tables aléatoires. Verdict PASS / FAIL / INCONCLUSIVE.'), lab('anneal', 'Recherche automatiquement la meilleure clé de substitution vers une langue cible (recuit simulé) ET la même recherche sur un texte de contrôle mélangé : seul un écart net avec le contrôle est un signal.'), lab('sukhotin', 'Algorithme de Sukhotin : identifie les symboles qui se comportent comme des voyelles.'), lab('hmm', 'Modèle de Markov caché : regroupe les symboles en classes selon leur comportement.'), lab('word_structure', 'Structure des mots : préfixes, suffixes, positions préférées, ordre relatif strict des glyphes (grammaire à cases).'), lab('keywords', 'Mots-clés par concentration (Montemurro & Zanette) : mots groupés dans certaines parties du manuscrit, avec leur section dominante.'), lab('similar_words', 'Mots aux contextes similaires (vecteurs de co-occurrence) : variantes, synonymes ou mots d’une même catégorie.'), lab('line_effects', 'Effets de position : glyphes et mots propres au début / fin de ligne, première ligne des paragraphes.'), lab('code', 'Exécute du JavaScript dans un bac à sable isolé. Données : VOYNICH (lignes {folio, locus, text, section, language, hand}), fonctions lines(filtre), words(filtre), freq(tableau), entropy(tableau), refWords(id) pour les corpus passés dans refs. Utiliser console.log et/ou return.'), tool('replay_experiment', 'Rejoue une expérience avec les mêmes paramètres pour vérifier un résultat annoncé (reproductibilité).', z.object({ id: z.number().int() }), async ({ id }) => JSON.stringify(await replayExperiment(id, author), null, 1)), tool('get_experiment', 'Affiche une expérience du journal (paramètres, critères, résultat).', z.object({ id: z.number().int() }), ({ id }) => {
        ...
        }), tool('add_crib', 'Ajoute un indice. La source est obligatoire (publication, étiquette visible, raisonnement iconographique).', z.object({
        ...
        }), (i) => `Indice #${addCrib({ ...i, authorLabel: ctx.agentName })} enregistré.`), lab('cribs', 'Applique une table de substitution aux indices et mesure la concordance.'), tool('crib_constraints', 'Déduit des indices les correspondances glyphe → lettre imposées (hypothèse de substitution simple) et signale les conflits.', z.object({ alphabet: z.enum(['eva', 'eva-grouped']).optional() }), ({ alphabet }) => JSON.stringify(cribConstraints(alphabet), null, 1)));
    }
    if (has('images')) {
        tools.push(tool('view_folio_image', 'Affiche l’image d’un folio du manuscrit (ou une zone, coordonnées relatives 0–1) pour l’examiner visuellement.', z.object({ folio: z.string().regex(/^f\d+[rv]\d*$/), region: REGION.optional(), size: z.number().int().min(400).max(2000).optional() }), async ({ folio, region, size }) => {
        ...
        }), tool('add_annotation', 'Annote une zone d’un folio (coordonnées relatives 0–1) : étiquette, plante, étoile, nymphe, signe du zodiaque…', z.object({
    ...
    if (ctx.genericWeb) {
        tools.push(tool('web_search', 'Recherche sur internet. Renvoie une liste de résultats (titre, URL, extrait). Utiliser ensuite fetch_url pour lire une page.', z.object({ query: z.string().min(2).max(300), limit: z.number().int().min(1).max(10).optional() }), async ({ query, limit }) => {
            const { engine, results } = await webSearch(query, limit ?? 6);
            if (!results.length)
                return `Aucun résultat (${engine}).`;
            return `Moteur : ${engine}\n\n${results.map((r, i) => `${i + 1}. ${r.title}\n   ${r.url}\n   ${r.snippet}`).join('\n\n')}`;
        }), tool('fetch_url', 'Lit le texte d’une page web ou d’un PDF en ligne (URL publique http/https). Utiliser offset pour lire la suite.', z.object({ url: z.string().url(), offset: z.number().int().min(0).optional() }), async ({ url, offset }) => {
    ...
    if (has('collaboration') && ctx.askAgent && ctx.otherAgents.length) {
        ...
        tools.push(tool('ask_agent', `Pose une question précise à un autre agent de l’équipe et récupère sa réponse (il peut utiliser ses propres outils). Agents disponibles : ${ctx.otherAgents.join(', ')}.`, z.object({ agent: z.string(), question: z.string().min(5).max(6000) }), ({ agent, question }) => askAgent(agent, question)));
    }
    return tools;
}
function requireCorpusFor(folio) {
    if (!corpus.corpusLoaded())
        throw new Error('Aucun corpus EVA importé.');
    if (!corpus.folioText(folio).length)
        throw new Error(`Folio ${folio} introuvable.`);
}
const EVIDENCE = /#E\d+|https?:\/\/\S+|doc\s*#\d+|\[[^\]]+,\s*\d{4}[^\]]*\]/i;
function hasEvidence(e) {
    return Boolean(e && EVIDENCE.test(e));
}
function experimentIds(e) {
