...
function sessionAgents(session) {
    const ids = JSON.parse(session.agent_ids);
    return ids.map(getAgent).filter((a) => Boolean(a && a.enabled));
}
function setStatus(sessionId, status, detail) {
...
function systemPrompt(agent, session, team) {
    ...
    return [
        ...
        capabilities(agent),
        `## Séance « ${session.title} »\nObjectif : ${session.objective}\nMode : ${session.mode === 'orchestrated' ? 'dirigé par un directeur de recherche qui délègue' : 'table ronde, chacun à son tour'}.`,
    ]
        .filter(Boolean)
        .join('\n\n');
}
...
function capabilities(agent) {
    ...
    const groups = JSON.parse(agent.tools);
    const lines = [];
    if (groups.includes('library'))
        lines.push("- **Bibliothèque interne** (search_library, read_document) : articles, livres, notes et fichiers partagés par l'équipe.");
    if (groups.includes('memory'))
        lines.push("- **Mémoire partagée** (search_memory, save_memory, update_memory) : acquis, hypothèses et impasses de l'équipe.");
    if (groups.includes('corpus'))
        lines.push('- **Corpus EVA** (corpus_get_folio, corpus_search, corpus_stats) : transcription du manuscrit et statistiques.');
    if (groups.includes('substitution'))
        lines.push('- **Tests de substitution** (apply_substitution).');
    if (groups.includes('collaboration'))
        lines.push('- **Consultation des autres agents** (ask_agent).');
    if (groups.includes('science'))
        lines.push("- **Laboratoire** : juge automatique (register_test, evaluate_decipherment), recherche de clé avec contrôle (anneal_substitution), empreintes comparées aux langues réelles, chiffres et textes générés (compare_fingerprints, list_reference_corpora), algorithmes (algo_sukhotin, algo_hmm, algo_word_structure, algo_keywords, algo_similar_words, algo_line_effects), calcul libre en JavaScript (run_code), indices (list_cribs, add_crib, test_cribs, crib_constraints), journal (get_experiment, replay_experiment). Chaque résultat porte un numéro #E<n> à citer.");
    if (groups.includes('images'))
        lines.push("- **Images du manuscrit** (view_folio_image, list_annotations, add_annotation) : examiner un folio ou une zone, relier une étiquette à une ligne de transcription.");
    if (agent.web_search)
        lines.push(`- **Recherche internet** (web_search${generic ? ', fetch_url pour lire une page' : ''}) : publications, bases de données, travaux récents sur le manuscrit.`);
    if (!lines.length)
        return '';
    return `## Tes sources et outils\n${lines.join('\n')}\n\nMéthode : consulte d'abord la mémoire et la bibliothèque internes${agent.web_search ? ", puis complète par une recherche internet ciblée quand une information manque ou doit être vérifiée. Privilégie les sources sérieuses (publications universitaires, voynich.nu, Beinecke Library) et cite toujours tes sources web (URL)" : ''}. Consigne dans la mémoire partagée ce qui mérite d'être retenu, avec sa source.`;
}
/** Les modèles sans recherche web native (fournisseurs compatibles OpenAI) utilisent nos outils web génériques. */
function usesGenericWeb(agent) {
    if (!agent.web_search || !agent.provider_id)
        return false;
...
function memoryDigest(session, recentText) {
    ...
    const parts = [];
    if (pinned.length)
        parts.push(`### Mémoire épinglée\n${pinned.map(formatMemory).join('\n\n')}`);
    if (relevant.length)
        parts.push(`### Mémoire pertinente\n${relevant.map(formatMemory).join('\n\n')}`);
    return parts.length ? `## Mémoire partagée de l'équipe\n${parts.join('\n\n')}` : '';
}
function contextParts(session) {
    const ids = JSON.parse(session.context_doc_ids);
    const parts = [];
    ...
    for (const id of ids) {
        ...
        if (doc.kind === 'image') {
            ...
            parts.push({
                type: 'text',
                text: `Document de contexte #${doc.id} : « ${doc.title} »${truncated ? ` (extrait ${take}/${doc.content.length} caractères — lire la suite avec read_document)` : ''}\n\n${doc.content.slice(0, take)}`,
            });
        }
    }
    return parts;
}
function buildHistory(session, agent, finalInstruction) {
    const rows = db
    ...
    for (let i = rows.length - 1; i >= 0; i--) {
        ...
        if (used > HISTORY_CHAR_BUDGET && kept.length > 0)
            break;
        kept.unshift(rows[i]);
    }
    ...
    push('user', [
        {
            type: 'text',
            text: `[Système] Ouverture de la séance « ${session.title} ».\nObjectif : ${session.objective}${omitted ? `\n(${omitted} messages plus anciens omis — consulter la mémoire partagée pour les acquis.)` : ''}`,
        },
        ...contextParts(session),
    ]);
    for (const r of kept) {
        if (r.kind === 'user')
            push('user', [{ type: 'text', text: `[Chercheur principal] : ${r.content}` }]);
        else if (r.kind === 'system')
            push('user', [{ type: 'text', text: `[Système] ${r.content}` }]);
        else if (r.agent_id === agent.id)
            push('assistant', [{ type: 'text', text: r.content }]);
        else
            push('user', [{ type: 'text', text: `[${r.agent_name ?? 'Agent'}] : ${r.content}` }]);
    }
...
async function runTurn(session, agent, opts) {
    const info = db
    ...
    const finish = (fields) => {
        if (opts.budget && fields.usage)
            opts.budget.used += fields.usage.inputTokens + fields.usage.outputTokens;
        db.prepare(`UPDATE messages SET content = ?, thinking = ?, error = ?, input_tokens = ?, output_tokens = ?, cache_read_tokens = ?, duration_ms = ?, model = COALESCE(?, model) WHERE id = ?`).run(fields.content, fields.thinking || null, fields.error ?? null, fields.usage?.inputTokens ?? 0, fields.usage?.outputTokens ?? 0, fields.usage?.cacheReadTokens ?? 0, Date.now() - started, fields.model ?? null, messageId);
        publish(session.id, { type: 'turn_end', tempId, message: getMessage(messageId) });
    };
    try {
        if (!agent.provider_id)
            throw new Error(`L'agent ${agent.name} n'a pas de fournisseur configuré.`);
        const providerRow = getProviderRow(agent.provider_id);
        if (!providerRow)
            throw new Error(`Fournisseur introuvable pour ${agent.name}.`);
        const provider = buildProvider(providerRow);
        const groups = JSON.parse(agent.tools).filter((g) => g in TOOL_GROUPS);
        ...
        const tools = buildTools({
            ...
            askAgent: opts.consultedBy
                ...
                : async (name, question) => {
                    ...
                    if (!target)
                        return `Agent « ${name} » inconnu. Agents disponibles : ${others.map((a) => a.name).join(', ')}`;
        ...
        const result = await provider.runAgentLoop({
            ...
            events: {
                ...
                onToolResult: ({ id, name, output, isError }) => {
                    db.prepare(`INSERT INTO messages(session_id, kind, agent_id, agent_name, tool_name, tool_input, tool_output, parent_id, error) VALUES (?, 'tool', ?, ?, ?, ?, ?, ?, ?)`).run(session.id, agent.id, agent.name, name, JSON.stringify(toolInputs.get(id) ?? null), output, messageId, isError ? 'erreur' : null);
        ...
        const error = aborted ? 'Interrompu par le chercheur' : err.message;
...
export function addSystemMessage(sessionId, content) {
...
function cycleRoles(session, team) {
    const judge = team.find((a) => /juge|judge|arbitre/i.test(`${a.name} ${a.role_title}`)) ?? team.find((a) => a.id === session.lead_agent_id) ?? team[team.length - 1];
...
export function startRun(sessionId, opts = {}) {
    ...
    if (!team.length)
        throw new Error('Aucun agent actif dans cette séance');
    const controller = new AbortController();
    running.set(sessionId, controller);
    setStatus(sessionId, 'running');
    const budget = { used: 0, limit: opts.tokenBudget ?? session.token_budget ?? null };
    (async () => {
        const signal = controller.signal;
        let finalStatus = 'idle';
        const turn = async (agent, instruction) => {
        ...
        try {
            if (opts.synthesize) {
                ...
                if (!agent)
                    throw new Error('Agent de synthèse introuvable');
            ...
            if (session.mode === 'cycle') {
                ...
                for (let r = 0; r < rounds && !signal.aborted; r++) {
                    for (const agent of order) {
                        if (signal.aborted)
                            break;
                        await turn(agent, `[Système] Tour ${r + 1}/${rounds} — c'est à toi, ${agent.name}. Réagis précisément aux dernières contributions (accord, désaccord argumenté, compléments), fais avancer la recherche selon ton rôle en utilisant tes outils quand c'est utile, puis termine par 1 à 3 propositions concrètes pour la suite.`);
                    }
                }
            }
        }
            ...
            if (err instanceof BudgetExceeded) {
                ...
                publish(sessionId, { type: 'error', error: err.message });
                finalStatus = 'error';
            }
        }
        ...
        if (opts.report && !signal.aborted) {
            try {
                ...
                publish(sessionId, { type: 'error', error: `Rapport : ${err.message}` });
            }
        }
