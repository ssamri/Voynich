...
export function folioFromLabel(label) {
...
export async function importManifest(url = DEFAULT_MANIFEST) {
    ...
    const manifest = JSON.parse(page.text);
    const canvases = manifest.items ?? manifest.sequences?.[0]?.canvases ?? [];
    if (!canvases.length)
        throw new Error('Manifeste IIIF sans pages (canvases).');
    const rows = [];
    const unmatched = [];
    ...
    db.transaction(() => {
        ...
        for (const r of rows)
            ins.run(r.folio, r.label, r.service, r.image, r.width, r.height, r.ord);
...
export function iiifInfo() {
    ...
    const n = db.prepare('SELECT COUNT(*) AS n FROM folio_images').get().n;
...
function iiifUrl(row, size, region) {
    if (!row.image_service) {
        if (!row.image_url)
            throw new Error(`Pas d’image pour ${row.folio}`);
...
export async function folioImage(folio, opts = {}) {
    const row = db.prepare('SELECT folio, image_service, image_url FROM folio_images WHERE folio = ?').get(folio);
    if (!row)
        throw new Error(`Aucune image pour ${folio} (importer le manifeste IIIF dans la page Manuscrit).`);
...
export function updateAnnotation(id, a) {
    const cur = db.prepare('SELECT * FROM annotations WHERE id = ?').get(id);
    if (!cur)
