...
import { DEFAULT_CRITERIA, evaluateDecipherment } from './evaluate.js';
import { getExperiment, runExperiment } from './experiments.js';
...
function voynichFilter(src) {
    const f = parseVoynichSource(src ?? 'voynich');
    if (!f)
        throw new Error('Cette analyse porte sur le corpus Voynich (source « voynich… »).');
    return f;
}
export async function runLab(kind, rawParams, author = {}) {
    const params = LAB_SCHEMAS[kind].parse(rawParams);
    const seed = typeof params.seed === 'number' ? params.seed : null;
    return runExperiment({ kind, title: author.title ?? TITLES[kind], params, seed, ...author }, async (criteria) => {
        switch (kind) {
            case 'fingerprint': {
                const target = { id: params.target, name: tokensFor(params.target).name, fp: fingerprintFor(params.target) };
                const refIds = params.references ?? [
                ...
                if (!refs.length)
                    throw new Error('Aucun corpus de comparaison : ajoutez des corpus de référence (page Laboratoire).');
            ...
            case 'anneal': {
                ...
                const r = anneal({
                    ...
                    cribs,
                    seed: params.seed,
                    controlSources: (params.control_sources ?? []).map((src) => {
...
export async function replayExperiment(id, author = {}) {
    const e = getExperiment(id);
    if (!e)
        throw new Error(`Expérience #${id} introuvable`);
    if (!(e.kind in LAB_SCHEMAS))
        throw new Error(`Type d’expérience non rejouable : ${e.kind}`);
    const replay = await runLab(e.kind, JSON.parse(e.params), { ...author, title: `Réplication de #${id} — ${e.title}` });
