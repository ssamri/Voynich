...
function allLines() {
    if (!lineCache) {
        lineCache = db
            .prepare(`SELECT l.folio, l.locus, l.text, p.illustration, p.language, p.hand
         FROM corpus_lines l LEFT JOIN corpus_pages p ON p.folio = l.folio ORDER BY l.ord`)
            .all();
    }
    return lineCache;
}
export function selectLines(f = {}) {
    const folios = f.folios?.length ? new Set(f.folios) : null;
    return allLines().filter((l) => (!f.illustration || l.illustration === f.illustration) &&
        (!f.language || l.language === f.language) &&
        (!f.hand || l.hand === f.hand) &&
        (!folios || folios.has(l.folio)));
}
export function wordsOf(lines, cleanOnly = true) {
    const out = [];
...
export function characterEntropy(text) {
    const chars = [...text];
...
export function ngrams(n, f = {}, limit = 50, level = 'char') {
...
export function compare(a, b, limit = 40) {
...
export function search(pattern, opts = {}) {
    ...
    if (opts.regex) {
        if (pattern.length > 200)
            throw new Error('Motif trop long');
    ...
    for (const l of lines) {
        ...
        if (m) {
            ...
            if (hits.length < limit)
                hits.push({ locus: l.locus, folio: l.folio, text: l.text, matches: m.slice(0, 10) });
        }
    }
...
export function folioText(folio) {
