...
export function importCorpus(content, source, transcriber = 'H') {
    ...
    db.transaction(() => {
        ...
        const ip = db.prepare('INSERT INTO corpus_pages(folio, ord, quire, panel, illustration, language, hand, header) VALUES (?,?,?,?,?,?,?,?)');
        ...
        for (const l of parsed.lines)
            il.run(l.folio, l.ord, l.locus, l.locusType ?? null, l.transcriber ?? null, l.raw, l.text);
        setSetting('corpus', JSON.stringify({ source, importedAt: new Date().toISOString(), transcriber, transcribers: parsed.transcribers }));
    })();
    invalidateCorpusCache();
    return corpusInfo();
}
export async function importCorpusFromUrl(url, transcriber = 'H') {
    const u = new URL(url);
...
export function corpusInfo() {
    ...
    const pages = db.prepare('SELECT COUNT(*) AS n FROM corpus_pages').get().n;
    const lines = db.prepare('SELECT COUNT(*) AS n FROM corpus_lines').get().n;
    return {
...
export function listPages() {
    return db
        .prepare(`SELECT p.*, (SELECT COUNT(*) FROM corpus_lines l WHERE l.folio = p.folio) AS line_count
       FROM corpus_pages p ORDER BY p.ord`)
        .all();
}
export function getPage(folio) {
