...
function pageVars(header) {
    const vars = {};
    for (const m of header.matchAll(/\$(\w)=(\w+)/g))
        vars[m[1]] = m[2];
    return vars;
}
/** Nettoie une ligne IVTFF et renvoie une suite de mots EVA séparés par des espaces. */
export function cleanIvtffText(raw) {
    let s = raw;
...
export function parseIvtff(content, preferredTranscriber = 'H') {
    const pages = new Map();
    ...
    for (const rawLine of content.split(/\r?\n/)) {
        ...
        if (lm) {
            ...
            if (!pages.has(folio))
                pages.set(folio, { folio, ord: pageOrd++, header: '' });
            if (transcriber)
...
export const ILLUSTRATION_LABELS = {
    A: 'Astronomique',
