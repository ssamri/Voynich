...
export const FREE_PRESETS = [
    {
        ...
        maxTokens: 8192,
        color: '#8b95a7',
        notes: 'Installez Ollama puis « ollama pull qwen3:14b ». Augmentez le contexte (OLLAMA_CONTEXT_LENGTH=32768), sinon les longues séances sont tronquées. Choisissez un modèle qui gère les outils (Qwen3, Llama 3.1, gpt-oss, Mistral).',
    },
    {
        id: 'lmstudio',
...
export function buildProvider(row) {
    ...
    if (hit && hit.stamp === row.updated_at)
        return hit.provider;
    const cfg = {
        id: row.id,
        name: row.name,
...
export function invalidateProvider(id) {
    cache.delete(id);
}
