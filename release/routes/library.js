...
import { createTextDocument, deleteDocument, documentFile, getDocument, ingestFile, searchLibrary, updateDocument, } from '../library/library.js';
...
libraryRouter.get('/', (req, res) => {
    ...
    const docs = db
        .prepare(`SELECT id, title, filename, mime, kind, size, tags, source, notes, created_at, length(content) AS chars
...
libraryRouter.post('/upload', upload.array('files', 20), async (req, res) => {
    ...
    for (const f of files) {
        try {
            ...
            results.push({ name: f.originalname, error: err.message });
        }
    }
