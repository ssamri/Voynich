...
import { FREE_PRESETS, PROVIDER_KINDS, buildProvider, getProviderRow, invalidateProvider } from '../llm/registry.js';
...
providersRouter.post('/', (req, res) => {
    ...
    const info = db
        ...
        .run(body.name, body.kind, body.baseUrl ?? null, body.apiKey ? encrypt(body.apiKey) : null, body.apiKey ? maskKey(body.apiKey) : null, body.defaultModel || PROVIDER_KINDS[body.kind].defaultModel || null);
...
providersRouter.put('/:id', (req, res) => {
    ...
    db.prepare(`UPDATE providers SET name=?, kind=?, base_url=?, api_key_enc=?, api_key_hint=?, default_model=?, updated_at=datetime('now') WHERE id=?`).run(body.name ?? cur.name, body.kind ?? cur.kind, body.baseUrl === undefined ? cur.base_url : body.baseUrl, body.apiKey ? encrypt(body.apiKey) : cur.api_key_enc, body.apiKey ? maskKey(body.apiKey) : cur.api_key_hint, body.defaultModel === undefined ? cur.default_model : body.defaultModel, id);
