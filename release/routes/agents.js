...
import { getAgent } from '../orchestrator/runner.js';
...
const toolKeys = Object.keys(TOOL_GROUPS);
...
agentsRouter.get('/', (_req, res) => {
    res.json(db.prepare('SELECT * FROM agents ORDER BY id').all().map(view));
});
function save(id, body) {
    ...
    if (id === null) {
        const info = db
            .prepare(`INSERT INTO agents(name, provider_id, model, role_title, system_prompt, temperature, max_tokens, effort, color, tools, web_search, enabled)
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`)
            .run(...values);
        return Number(info.lastInsertRowid);
    }
    db.prepare(`UPDATE agents SET name=?, provider_id=?, model=?, role_title=?, system_prompt=?, temperature=?, max_tokens=?, effort=?, color=?, tools=?, web_search=?, enabled=?, updated_at=datetime('now') WHERE id=?`).run(...values, id);
    return id;
}
