...
import { MEMORY_LABELS, MEMORY_TYPES, createMemory, searchMemories, updateMemory } from '../memory.js';
...
memoryRouter.post('/', (req, res) => {
    ...
    if (body.status && body.status !== 'active')
        res.json(updateMemory(m.id, { status: body.status }));
    else
        res.json(m);
});
...
memoryRouter.get('/export', (_req, res) => {
    const rows = db.prepare('SELECT * FROM memories ORDER BY type, id').all();
